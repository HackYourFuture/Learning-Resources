...
Before you start tinkering with the code it is essential that you build an understanding of how the application's functionality and structure. Here, VS Code Copilot can be of great help.
...
3. We will be asking Copilot to explain in the code this folder. Ensure that the drop-down below the input field is set to **Ask**. We don't want Copilot to make changes to our code.
...
6. With the Copilot explanation now at hand, go through all files and try to make sense of the code, and in particular `ui.js` and `index.js`. If you want, you can follow-up questions to Copilot to explain particular code fragments in more detail.
...
You will notice that there is some repeated code in this application. Each of the function that call `fetch()` contains repeated code (the code is not DRY). Refactor the code in the following manner:
