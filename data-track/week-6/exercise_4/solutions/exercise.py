...
def build_create_command() -> str:
    ...
    return (
        ...
        f"  --replica-timeout {REPLICA_TIMEOUT_SECONDS} \\\n"
        f"  --image {IMAGE} \\\n"
        f"  --registry-server {REGISTRY_SERVER} \\\n"
        f"  --container-name {CONTAINER_NAME} \\\n"
        f"  --env-vars {env_vars_joined}"
    )
# WHY a multi-line string with backslash continuations: the same command can be
# copy-pasted directly into a Bash shell. A single-line string is also valid Bash,
# but unreadable in code review. The TODO_self_check operates on the joined string,
# so either format passes the test; readability is the deciding factor.
#
# WHY include --container-name explicitly: az containerapp job create accepts the
# flag, but if you omit it the CLI silently invents one from the image tag, and
# every subsequent `az containerapp job update --container-name X` then fails
# because X does not match the auto-generated name. Set it once, set it
# explicitly. This is the most common "I changed nothing and it broke" failure.
#
# WHY secretref: for env vars: Container Apps stores secrets at the job level and
# resolves `secretref:postgres-url` at startup. Putting plain-text secrets in
# --env-vars writes them into the job's resource definition, where anyone with
# `az containerapp job show` access can read them.
